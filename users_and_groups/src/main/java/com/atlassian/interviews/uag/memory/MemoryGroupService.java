...
public class MemoryGroupService extends AbstractService implements GroupService {
	...
	private MembershipService memoryMembershipService;

	public MemoryGroupService(Services services) {
		super(services);
		memoryMembershipService = services.getMembershipService();
	}
	...
	public void delete(Group group) {
		requireNonNull(group, "group");
		Collection<User> users = memoryMembershipService.getUsersInGroup(group);
		for (User user : users) {
			memoryMembershipService.removeUserFromGroup(user, group);
		}
